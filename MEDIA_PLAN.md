# Media Plan — Salewave Site

## Goals
- Reinforce trust and clarity with simple, brand-aligned visuals.
- Ensure every primary page includes at least one meaningful image.
- Embed a short video for motion and storytelling (hosted).

## Page-by-Page Plan

### Home (Hero + Overview)
- **Hero background:** `static/images/hero/hero-background.jpg`
- **Purpose:** Abstract tech background to reinforce modern AI positioning.
- **Placement:** Home hero section.

### Services (Overview)
- **Banner image:** (removed)
- **Purpose:** Keep layout clean; rely on service card icons.

### Service Detail Pages
- **CRM Automation Consulting:** `static/images/services/crm-automation-consulting.jpg`
- **Data Integration Consulting:** `static/images/services/data-integration-consulting.jpg`
- **CX Consulting:** `static/images/services/cx-consulting.jpg`
- **Purpose:** Provide contextual visuals that anchor consulting offerings.
- **Placement:** First block in each consulting service page.

### About
- **Image:** (removed)
- **Video:** (removed)

### Blog
- **Banner image:** (removed)
- **Purpose:** Minimal header for readability.

### Use Cases
- **Healthcare:** `static/images/use-cases/healthcare.jpg`
- **Legal:** `static/images/use-cases/legal.jpg`
- **Hospitality:** `static/images/use-cases/hospitality.jpg`
- **Fitness:** `static/images/use-cases/fitness.jpg`

## Accessibility
- Every image includes descriptive `alt` text and a caption.
- Video includes fallback text and uses a descriptive caption.

## Optimization
- JPEGs are resized to 1600px width for consistent delivery.

## Source Notes
- Photo assets are sourced from Unsplash (free license) and downloaded at 1600px width.
  - Hero background: https://unsplash.com/photos/ZhVUAUc8V4s
  - CRM automation consulting: https://unsplash.com/photos/yd_RKGH_RH4
  - Data integration consulting: https://unsplash.com/photos/WbLJd4M88I8
  - CX consulting: https://unsplash.com/photos/qoLY6vRnJsE
  - Healthcare: https://unsplash.com/photos/L8tWZT4CcVQ
  - Legal: https://unsplash.com/photos/VW5VjskNXZ8
  - Hospitality: https://unsplash.com/photos/poI7DelFiVA
  - Fitness: https://unsplash.com/photos/0sa8D74iodI
